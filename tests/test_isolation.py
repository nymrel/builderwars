...
class IsolationProfileTests(unittest.TestCase):
    ...
    def test_scalar_bool_aliases_are_rejected(self):
        for field, value in (("capability_isolation", 0), ("trusted_entrants_only", 1)):
            forged = resolve_isolation()
            forged[field] = value
            with self.assertRaisesRegex(ValueError, field):
                validate_isolation_profile(forged)
        forged = resolve_isolation()
        forged["unenforced"]["network_egress_blocking"] = 0
        with self.assertRaisesRegex(ValueError, "unenforced controls"):
            validate_isolation_profile(forged)
...
class IsolationReplayTests(unittest.TestCase):
    ...
    def test_legacy_policy_rejects_added_control_claim(self):
        with tempfile.TemporaryDirectory() as temporary:
            result = run_match(game_name="nim", seed=8, entrants=[entrant("solver_harness.py"), entrant("naive_harness.py")], out_dir=temporary)
            transcript = Path(result["transcript"])
            def mutate(records):
                records[0]["body"]["sandbox_policy"] = legacy_policy()
                records[0]["body"]["sandbox_policy"]["unenforced_v1"].append("host_credential_boundary false")
                records[0]["body"].pop("isolation", None)
            rechain(transcript, mutate)
            self.assertEqual(verify(transcript)["verdict"], "FAIL")
