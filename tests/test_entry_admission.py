...
class EntrantAdmissionTests(unittest.TestCase):
    ...
    def test_run_series_refuses_external_entries_before_execution(self) -> None:
        output = Path(self.temporary.name) / "series"
        result = subprocess.run(
            [sys.executable, str(BIN / "run_series.py"), "--a", str(self.external_script),
             "--b", str(self.external_script), "--seeds", "1", "--out", str(output)],
            cwd=ROOT, capture_output=True, text=True, timeout=30,
        )
        self.assertEqual(result.returncode, 2)
        self.assertIn("--allow-unconfined-entrants", result.stderr)
        self.assertFalse(output.exists())

    def test_bundled_clis_keep_current_customer_local_contract(self) -> None:
        entrant_a = str(ROOT / "entrants" / "solver_harness.py")
        entrant_b = str(ROOT / "entrants" / "naive_harness.py")
        commands = (
            ["run_match.py", "--seed", "9911", "--entrant", entrant_a, "--entrant", entrant_b],
            ["run_series.py", "--a", entrant_a, "--b", entrant_b, "--seeds", "1"],
        )
        for index, command in enumerate(commands):
            with self.subTest(command=command[0]):
                output = Path(self.temporary.name) / f"bundled-{index}"
                result = subprocess.run(
                    [sys.executable, str(BIN / command[0]), *command[1:],
                     "--backend", "stub:v1", "--out", str(output)],
                    cwd=ROOT, capture_output=True, text=True, timeout=30,
                )
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertTrue(output.exists())

    def test_module_invocation_remains_supported(self) -> None:
        for name in ("run_match", "run_series"):
            with self.subTest(module=name):
                result = subprocess.run(
                    [sys.executable, "-m", f"bin.{name}", "--help"],
                    cwd=ROOT, capture_output=True, text=True, timeout=30,
                )
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertIn("--allow-unconfined-entrants", result.stdout)
