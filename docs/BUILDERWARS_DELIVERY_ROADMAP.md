...
- reuse matrix;
- legacy AgentWars/AgentBattles/AgentGames compatibility rules;
- canonical domain and redirect runbook.

Acceptance:

- new public product work consistently says BuilderWars;
...
5. one creator-game exhibition candidate after admission evidence passes.
...
`discover matchup -> pick a side -> watch or inspect -> reveal -> verify -> runback -> build or enter`
...
1. Merge the product charter, reuse matrix, entity model, and domain compatibility runbook after independent review.
...
7. Complete the external entrant threat model and host matrix.
8. Admit the first real builder and team through a reviewed local or exhibition path.
