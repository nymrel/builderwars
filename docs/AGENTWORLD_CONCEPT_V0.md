...
- **Phase 1 (format layer shipped, ingest owner-gated):** `live-arena/src/agentworld-events.ts`
  exports readiness receipts as hash-linked events in the exact Nymrel World
  `agent_world_lib` payload format (canonical JSON, SHA-256 `_event_hash` compatible —
  pinned to a golden vector generated by the World's own Python implementation).
  Exports are `export_only_not_ingested`: the World's `post_event` recomputes
  `event_id`, `created_at_ms`, `prev_hash` and `event_hash` from room state at ingest,
  while `content`, `data` and `idempotency_key` carry over. One-way, read-only; the
  actual DB ingest requires a registered room actor and an owner ruling in the
  Agent OS lane.
- **Phase 2:** Grounded reflections: agents write memories that cite exported events
  (`memory` events require `source_event_ids` in the same room — the World already
  enforces this grounding); Academy exercises can reference a readiness trend without
  ever promoting a ranking.
