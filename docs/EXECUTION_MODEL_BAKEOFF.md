...
Each request clears the preceding receipt. A billed response with invalid or
empty assistant text retains its own reported usage while the move falls back;
a transport failure never repeats the previous request's cost. Redirects are
refused before an authorization header can be forwarded to another endpoint.

The command uses the customer-local execution wrapper and provisions only the
selected credential name into each entrant. Existing summary files are refused
before any match runs. Exit 0 means every accepted move was model-claimed;
exit 2 means replay passed with mixed, partial, or fallback-only play. These
states remain explicitly unattested and are distinguished in the summary.
