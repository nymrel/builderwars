...
BuilderWars has a strong local reference foundation: exact schemas, tenant predicates, a fail-closed browser-authorization gateway, atomic owner-scoped browser idempotency with versioned encrypted replay and a bounded rotation keyring, signed runner requests, durable nonce consumption, transactional lease and deletion behavior, deterministic replay, an engine-versioned reviewed-reference source registry, bounded publication projections, and source-bound local launch evidence. Those controls are real and testable, but the Clerk verifier, durable perimeter, production-store parity, protected key custody/rotation execution, and deployment adapters do not yet exist.
...
| C-004 | Hosted state store | Tenant, browser-idempotency, runner, nonce, lease, result, and projection state | Transactional SQLite reference only |
...
| C-008 | Entrant process boundary | JSONL subprocess channel, exact reviewed-source admission, limits, and tree cleanup | Partial; explicitly not OS isolation |
...
| B-006 | Arena referee -> entrant process | `arena/1` JSONL moves through stdin/stdout | Engine-versioned path/digest allowlist for every reviewed entrant entrypoint and shared entrant source dependency; separate customer-local unreviewed scope; scratch cwd; environment allowlist; timeouts; output caps; tree cleanup | Network, filesystem, CPU, and memory are not confined; a source swap after preflight is not excluded; external runtime and third-party import identity remain unattested |
...
| --- | --- | --- | --- | --- |
| EP-001 | Owner-authenticated hosted commands | Verified browser-principal reference | B-001 | Create, confirm, revoke, delete, and fixture operations pass the local gateway with atomic local retry replay but still require production Clerk and store parity |
...
| EP-006 | Arena entrant manifest | Registry-bound reviewed reference or customer-controlled local subprocess command | B-006 | `builderwars/entrant-admission/1` binds the local scope; `builderwars/reviewed-reference-sources/1` requires exact reviewed entrypoint and shared entrant-source bytes; customer-local source stays explicitly unreviewed; hosted-untrusted execution is refused before side effects and remains unsafe without a jail |
...
| TM-005 | Malicious entrant or creator | Shared host runs attacker-controlled command | Access host capabilities or exhaust resources | Credential theft, host compromise, denial, lateral movement | Required receipt-bound local scope; hosted-untrusted fail-closed admission; exact engine-versioned path/digest preflight for reviewed entrypoints and shared entrant sources; customer-local source remains unreviewed; scratch cwd, env allowlist, timeout, caps, tree cleanup | No network, filesystem, CPU, memory, or POSIX escape confinement; a post-preflight source swap is not excluded; external runtime and third-party import identity remain unattested | Keep public code disabled; require disposable OS jail, egress controls, quotas, independent escape tests | Admission refusal/no-side-effect proof, reviewed-source registry/digest mismatches, sandbox profile, limit exits, egress denials, orphan checks | Low while disabled; high when enabled | High | High, protected hold |
...
| `arena/admission.py` | Required local execution scopes and pre-side-effect hosted-untrusted refusal | TM-005 |
| `arena/reference_sources.py` | Engine-versioned path and SHA-256 authority for reviewed entrypoints and shared entrant source dependencies | TM-005, TM-010 |
...
The browser boundary, adapter checklist, focused adversarial command, and rollback expectations are specified in [`AGENTWARS_BROWSER_AUTHORIZATION_BOUNDARY.md`](AGENTWARS_BROWSER_AUTHORIZATION_BOUNDARY.md). The executable model binds 18 exact source anchors; all production authority remains false.
