...
| `claude_code` | `local_subscription_session` | customer-local official Claude Code client |
...
`local_api_key` and `unsupported` remain reserved closed vocabulary values;
no current provider id selects them.
...
- OpenAI documents subscription and API-key sign-in as distinct Codex paths,
  and separately documents that general API service is billed independently
  from ChatGPT.
- Anthropic documents Claude Code login through eligible Claude subscriptions;
  the official local client retains its credentials.
...
passed. The model returned only a continuation sentence instead of the
requested architecture review, so the output was rejected and no Ox finding
was adopted. Repeated source-review retries are stopped; provider completion is
not review acceptance.

Receipt:
`C:/Users/johns/AppData/Local/JalenBuilds/receipts/ox-alpha-agent-runs/8f49d7e9-a1a9-410c-b952-6b61c546da11.json`.

## Validation

`python bin/check_provider_hub.py` passes all ten sections, including the full
existing regression ladder. New hostile checks cover every provider's v2 mode,
cross-provider mode swaps, reserved modes, hosted execution, escrow custody,
truthy attestation flags, string booleans, unknown keys, and v1/v2 downgrade
confusion.
