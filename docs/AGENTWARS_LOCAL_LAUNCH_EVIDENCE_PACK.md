...
| 9 | Hosted security, abuse, cleanup, browser authorization, and threat model | Runs local hosted-control-plane refusal, rollback, cleanup, browser-gateway adversarial tests, and the repository-grounded 10-threat/8-boundary security model | Live Clerk/session verification, durable production store/rate limits/deletion/idempotency, owner-pepper custody, OS isolation, penetration review, and security approval |
