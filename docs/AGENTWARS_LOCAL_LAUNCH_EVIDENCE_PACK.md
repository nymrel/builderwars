...
| 7 | Mobile static contracts | Checks deterministic mobile, truth, offline, accessibility, exact game/version/format/engine/resource-scoped proof-rating contracts, and the source-bound append-only correction overlay; rows remain alphabetic and ranking-ineligible, while corrected receipts stay in history but cannot influence current scoped proof points | Hosted route, real user, deployed device support, cross-scope comparison, global ranking |
