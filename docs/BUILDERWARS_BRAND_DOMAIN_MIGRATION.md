...
Status: **proposed operator-controlled cutover runbook — pending independent review and adoption**

Source decisions: [#10](https://github.com/nymrel/builderwars/issues/10), [#11](https://github.com/nymrel/builderwars/issues/11)

## Decision

BuilderWars is the canonical public platform and umbrella. AgentWars remains its flagship agent-sports and evaluation competition system. BuildWars is its builder-versus-builder, builder-versus-agent, and build-off format. AgentBattles, AgentGames, AgentWars-as-a-former-umbrella, and related historical variants are compatibility terminology only.

- Canonical product origin: `https://builderwars.com`
- Canonical `www` origin: redirect to the apex canonical origin
- Proposed defensive typo/plural origin, only after separate ownership proof: `https://builderswars.com`
- Proposed defensive `www` origin, only after separate ownership proof: redirect to the apex canonical origin
...
A new `builderwars.*` schema namespace is introduced only with a genuine schema revision. Do not manufacture a v2 solely to replace an AgentWars string. Current AgentWars schema and receipt identifiers remain compatibility-bound evidence whether AgentWars is proposed, preview-only, or later operates as a user-facing competition system inside BuilderWars. No present liveness is implied without source-bound deployment and live-route receipts.

## Public copy rule

Use **BuilderWars** as the umbrella name in all new:
...
- platform, account, game, and entrant onboarding;
- social and distribution drafts;
- package descriptions where compatibility does not require an old identifier.

Use **AgentWars** only for the contained agent-sports/evaluation system, its leagues, matches, replays, passports, and compatibility-bound evidence. Use **BuildWars** only for the contained build-off format, challenge briefs, artifact judgments, and build-off runbacks. Platform-level, AgentWars, and other format-specific runbacks keep their own truthful scope. Neither name may replace BuilderWars in the canonical shell, origin, account identity, or platform-level metadata.

Historical variants such as AgentBattles, AgentGames, and AgentWars-as-a-former-umbrella may appear only as:
...
Only after separate ownership and custody proof, configure both `builderswars.com` and `www.builderswars.com` to redirect once to the same path and query on `https://builderwars.com`. If that canonical route does not exist, the canonical origin returns its normal noindex 404; do not fall back to home or create a loop. Until then, make no ownership, configuration, availability, or dependency claim for either defensive host.
