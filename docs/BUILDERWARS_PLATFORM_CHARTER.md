...
Status: **proposed product source of truth — pending independent review and adoption**
...
- Public platform and umbrella: **BuilderWars**
- Flagship agent-sports and evaluation system: **AgentWars**
- Builder-versus-builder, builder-versus-agent, and build-off format: **BuildWars**
- Canonical domain: `builderwars.com`
- Proposed defensive domain: `builderswars.com`, only after separate ownership proof
- Parent company and studio: **Nymrel**
- AgentBattles, AgentGames, AgentWars-as-a-former-umbrella, and similar historical variants become compatibility terminology only.

Existing digest-bound receipts, verifier snapshots, schema namespaces, manifest identifiers, and historical routes must not be renamed merely for branding. New public shells, navigation, and platform-level copy use BuilderWars. AgentWars and BuildWars remain allowed only for their defined contained competition system and format; neither becomes a second platform or canonical origin.
...
This is the spectator subset. Delivery roadmap B2 may append the gated participation tail `build or enter` only after verification and runback; that tail is not evidence that an account, entrant, or public builder flow is live.
...
`Tenant` is the private authorization and data-isolation boundary. `Run` is one immutable participant execution inside a `Match`. `Publication` is a rights-reviewed, redacted public projection of accepted private evidence. The canonical entity model defines their full fields, versions, state machines, and receipt bindings before implementation.

Authentication, public identity, tenant authority, competition roles, review,
publication, and operator launch authority are separate grants. A team name is
not evidence of its match roster. A model label is not independent model
attestation. A provider route is not a model identity. A replayable result
proves the match that occurred, not every origin claim attached to the entrants.
Account- and tenant-bearing public admission remains held until the D3 and D4
contracts and their exact implementation evidence pass.
...
1. the same-model harness duel, pending exact A2 component-acceptance evidence;
...
5. one held creator-game exhibition only after Workstream E admission machinery and exact implementation evidence pass.
...
Nymrel operates BuilderWars and selects official circuits, but open contracts should allow third parties to create compatible entrants, games, verifiers, analytics, and spectator tools. Third-party verification tools remain unofficial until a versioned governance, security, parity, revocation, and result-custody gate explicitly accepts them; compatibility alone never grants official verifier authority.
