...
export function validateRules(raw: unknown): Rules {
  ...
  if (r.kind === "nim") {
    ...
    if (
      !Array.isArray(heaps) ||
      ![3, 4].includes(heaps.length) ||
      !heaps.every((heap) => Number.isInteger(heap) && heap >= 1 && heap <= 7) ||
      heaps.reduce((xor, heap) => xor ^ heap, 0) === 0
    )
...
export function moveLabel(move: string, s: GameState): string {
  if (s.rules.kind === "nim") {
    const { heap, take } = JSON.parse(move) as { heap: number; take: number };
...
export function botMove(s: GameState, style = "tactician"): string {
  ...
  if (s.rules.kind === "nim") {
    const heaps = nimHeaps(s), xor = heaps.reduce((value, heap) => value ^ heap, 0);
    const heap = heaps.findIndex((count) => (count ^ xor) < count);
    return heap < 0
      ? moves[0]
      : JSON.stringify({ heap, take: heaps[heap] - (heaps[heap] ^ xor) });
  }
  const player = s.turn;
  let best = -Infinity,
    chosen = moves[0];
