...
def validate_isolation_profile(profile):
    ...
    for field, expected in expected_scalars.items():
        if not _exact_equal(profile[field], expected):
            raise ValueError(
    ...
    if not _exact_equal(enforced, PROCESS_ISOLATION["enforced"]):
        raise ValueError("enforced controls do not match the implemented process boundary")
    if not _exact_equal(unenforced, PROCESS_ISOLATION["unenforced"]):
