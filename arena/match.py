...
_WINDOWS_DEVICE_NAMES = frozenset({"CON", "PRN", "AUX", "NUL"} | {f"COM{n}" for n in range(1, 10)} | {f"LPT{n}" for n in range(1, 10)})


def validate_match_id(value):
    if not isinstance(value, str) or _MATCH_ID.fullmatch(value) is None:
        raise ValueError("match_id must be 1-80 ASCII letters, digits, underscores, or hyphens and must start with a letter or digit")
...
def run_match(
    *,
    ...
    mid = validate_match_id(match_id_for(game_name, seed, [e["name"] for e in entrants]) if match_id is None else match_id)
...
def run_reference_match(**kwargs):
    """Run a bundled reference pair through the same local process profile."""
    return run_match(**kwargs)


def run_customer_local_match(**kwargs):
    """Run a customer-local pair through the same local process profile."""
    return run_match(**kwargs)
