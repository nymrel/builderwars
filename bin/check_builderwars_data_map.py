...
def main() -> int:
    ...
    check(cache_match is not None and cache_match.group(1) == "builderwars-mobile-arena-v40", "current service-worker cache is discovered")
    check(any(row["referenceLocation"] == "CacheStorage:builderwars-mobile-arena-v40" for row in data_sets), "current service-worker cache is mapped")
