...
from run_agentwars_cross_provider_match import reserve_json_output  # noqa: E402
...
def main(argv=None):
    ...
    reservation = reserve_json_output(args.json_out) if args.json_out else None
    try:
        return run(args, entrants, reservation)
    finally:
        if reservation is not None:
            reservation.abort()


def run(args, entrants, reservation=None):
    result = run_match(
        game_name=args.game,
        seed=args.seed,
        entrants=entrants,
        provisioned_envs=[{args.api_key_env: os.environ[args.api_key_env]} for _ in entrants],
        out_dir=args.out,
    ...
    all_model = both_used_model and all(
        sum(counts.values()) == counts["model"] for counts in sources.values()
    )
    status = (
        "model_influenced_unattested" if all_model
        else "mixed_model_and_fallback_unattested" if both_used_model
        else "partial_model_influence_unattested" if any(model_move_claims.values())
        else "fallback_only_not_model_played"
    )
    summary = {
